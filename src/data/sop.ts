...
export const incidentSop: Record<IncidentType, SopStep[]> = {
  ...
  blackout: [
    { owner: 'security', action: '立即切换应急视图，确认门禁是否失效；失效时第一时间到场，启用机械钥匙或临时开门，读者端推送疏散路线与集合点（不能只依赖扫码出门），稳住读者防止踩踏' },
    { owner: 'admin', action: '广播说明并引导疏散，按区域清点在馆人数；核对应急照明、疏散指示灯、消防主机、烟感状态；借还请求转人工暂存（记录操作时间与设备编号）' },
    { owner: 'security', action: '若有人被困、消防通道被占、烟感离线或应急灯不亮，立即升级为紧急事件，同步街道值班与消防联系人（119），巡检状态不允许完成' },
    { owner: 'maintainer', action: '联系供电与物业，确认电梯困人等次生风险；设备离线期间挂账，来电后逐项自检（图书消磁、还书箱、自助机），故障设备进入跨日交接' },
    { owner: 'service', action: '饮水机、打印机停止服务同步读者端；空调停运超阈值时由管理员评估提前闭馆并通知已预约读者；来电后按操作时间和设备编号补录借还，避免误算逾期/借阅失败' },
    { owner: 'street', action: '街道值班查看停电小区、影响书房、在馆人数与处置进展；夜间停电后管理员与安保逐项确认人员清场、门窗、消防通道、应急照明、门禁恢复、摄像头回传，未恢复不得标记闭馆完成；事后复盘停电时间、影响范围、读者求助、处置责任与改进项' }
  ],
  complaint: [
